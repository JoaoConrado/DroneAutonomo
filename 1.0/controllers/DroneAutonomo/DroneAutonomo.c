...
void calcularDiferenca(double x_atual, double y_atual, double x_desejado, double y_desejado, double* pitch, double* roll) {
	...
	*pitch = CLAMP(kp * deltaX + ki * deltaX + kd * deltaX, -2, 2);
	*roll = -CLAMP(kp * deltaY + ki * deltaY + kd * deltaY, -2, 2);
}
...
int main(int argc, char** argv) {
	...
	while (wb_robot_step(timestep) != -1) {
		...
		if (time > 10) {
			calcularDiferenca(X, Y, arrayDeCoordenadas[pontoAtual].x, arrayDeCoordenadas[pontoAtual].y, &pitch_disturbance, &roll_disturbance);
			printf("x: %f y: %f z: %f pitchDisturbance: %f rollDisturbance: %f ponto: %d\n", X, Y, altitude, pitch_disturbance, roll_disturbance, pontoAtual);
